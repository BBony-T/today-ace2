...
const generationConfig = {
  ...
  topP: 0.9,
  topK: 40,
  maxOutputTokens: 480, // 충분히 길지만 과하지 않게
};
...
function buildPrompt({ career, statsSummary, stats, username, rosterId, studentName, short=false }) {
  ...
  const system = [
    ...
    '명령조 대신 제안/권유형 어조를 쓰고, 부담스럽지 않은 작은 실천을 제안해.',
    '먼저 최다 추천 역량에 대한 칭찬과 구체적 강점을 2~3문장,',
    '다음으로 관심 진로나 활동과 연결해 3~5가지 실천 아이디어를 제시해.',
    '불필요한 공백 줄 없이 자연스럽게 작성.'
  ].join(' ');
